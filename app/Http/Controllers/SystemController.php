...
//                }
    ...
    public function companyIndex(Request $request )
    {
        if(\Auth::user()->can('manage company settings'))
        {
            ...
            $offerlangName = Language::where('code',$offerlang)->first();
            $joininglangName = Language::where('code',$joininglang)->first();
            $explangName = Language::where('code',$explang)->first();
            $noclangName = Language::where('code',$noclang)->first();
...
//            $ip = $_SERVER['REMOTE_ADDR'];
    ...
    public function chatgptSetting(Request $request)
    {
        ...
        if(isset($request->ai_chatgpt_enable)  && $request->ai_chatgpt_enable == 'on')
        {
            ...
            $post['ai_chatgpt_enable']            = 'on';
        }else{
            $post['ai_chatgpt_enable']            = 'off';
        }
